package in.hoptec.ppoweradmin.utils;
...
import in.hoptec.ppoweradmin.utl;
...
public class ExecuterU extends AsyncTask<Void,Void,Void> {
    ...
    public ExecuterU( Context ctx, ProgressDialog message)
    {
        this.ctx=ctx;
        this.r=r;
        this.pd=message;
...
}catch (Exception e)
{
    ...
    private void copyAssets() {
        ...
        for(int i=0;i<raw.length;i++) {
            InputStream in = ctx.getResources().openRawResource(raw[i]);
            try {
                File f2=new File(folder+"/assets.zip");
                ...
                try{
                    ...
                    in.close();
                    out.close();
                    String zipFile =folder + "/assets.zip" ;
                    String unzipLocation = folder + "/" ;
                    /*Decompress d = new Decompress(zipFile, unzipLocation);
                    d. unzip();

                */}
            } catch (IOException e) {
                utl.toast(ctx,"BUFF ERR");
                e.printStackTrace();
            }
        }

//        utl.toast(ctx, "Assets d extracted !");
        Log.d("i", "i");
    }
